...
class Keywords {
	  ...
	  static public function Get($id=null)
        {
                ...
                if(isset($id))
                {
                        return fetch_one("SELECT * FROM Fall2013_KeyWords WHERE id=$id");                        
                }
                else
                {
	...
	static public function GetSelectListFor($id)
   {
	 	...
	 	static public function Save($row)
        {
        	   ...
        	   if($row['id'])
			   {
			   	 $sql = " UPDATE  Fall2013_KeyWords " 
			   	 . " Set Name='$row2[Name]' " 
			   	 . " WHERE id=$row2[id] " ;
			   }
			   else {
				   $sql = " Insert Into Fall2013_KeyWords (Name) "
                        .  " Values ( '$row2[Name]') ";
			   }
	...
	{
				$conn = GetConnection();
                $sql =  " DELETE From Fall2013_KeyWords  WHERE id=$id ";
